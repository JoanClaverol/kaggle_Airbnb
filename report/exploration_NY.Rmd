...
# cheking the metrics of the model 
# checking at the metrics
words_data %>% 
  add_predictions(model = mod_price_dummy, var = "price_pred") %>% 
  add_residuals(model = mod_price_dummy, var = "price_resid") -> temp
postResample(pred = temp$price_pred, obs = temp$price)
```
...
Try to use caret with a random forest to predict the prices:

```{r}
names <- words_data %>% 
    select(neighbourhood_group, room_type, price,starts_with("word_filter_"))  %>% 
    names()

source("models/fun_model_creation.R")
model_caret <- modalization(
  data = words_data, p_partition = 0.8, cv_repeats = 1, cv_number = 1,
  x = "price", y = names, model_name = "rf", set_seed = 123
  )
